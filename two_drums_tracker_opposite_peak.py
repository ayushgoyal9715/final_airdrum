...
def calculate_positions_and_velocity(df, delta_t=0.01, reset_interval=20):
    df = df.copy()
    df['v_x'] = 0.0
    df['v_y'] = 0.0
    df['v_z'] = 0.0
...
# Define drum boundaries and other constants for drumset 1
drum_boundaries_1 = {
    'hitoms': {'x_min': -4.5, 'y_min': 0, 'x_max': -1.5, 'y_max': 1},
    'ride': {'x_min': -1.5, 'y_min': 0, 'x_max': 1.5, 'y_max': 1},
    'lotoms': {'x_min': 1.5, 'y_min': 0, 'x_max': 4.5, 'y_max': 1},
    'hihats': {'x_min': -4.5, 'y_min': -1, 'x_max': -1.5, 'y_max': 0},
    'snare': {'x_min': -1.5, 'y_min': -1, 'x_max': 1.5, 'y_max': 0},
    'crash': {'x_min': 1.5, 'y_min': -1, 'x_max': 4.5, 'y_max': 0}
}

# Define drum boundaries and other constants for drumset 2
drum_boundaries_2 = {
    'lotoms': {'x_min': -4.5, 'y_min': 0, 'x_max': -1.5, 'y_max': 1},
    'hihats': {'x_min': -1.5, 'y_min': 0, 'x_max': 1.5, 'y_max': 1},
    'lotoms': {'x_min': 1.5, 'y_min': 0, 'x_max': 4.5, 'y_max': 1},
    'ride': {'x_min': -4.5, 'y_min': -1, 'x_max': -1.5, 'y_max': 0},
    'crash': {'x_min': -1.5, 'y_min': -1, 'x_max': 1.5, 'y_max': 0},
    'snare': {'x_min': 1.5, 'y_min': -1, 'x_max': 4.5, 'y_max': 0}
}
z_threshold = -5

# Find the drum type
def find_drum_type(df, peak, drum_boundaries):
    detected_drum = None
...
# Process drum hits
def give_drumtype_and_level(df, peaks, drum_boundaries):
    pygame.init()
    pygame.mixer.init()
    for peak in peaks:
        drum_type = find_drum_type(df, peak, drum_boundaries)
        sound_level = find_drum_sound_level(df, peak)
        print(peak, drum_type, sound_level)
        if drum_type:
            play_sound_thread(drum_type, sound_level)
        time.sleep(0.2)  # Reduced sleep time for faster processing

# Read and process CSV in real-time
def read_and_process_csv(csv_file, drum_boundaries, interval=1, buffer_size=100, reset_interval=20):
    buffer = pd.DataFrame()
    ...
    while True:
        ...
        if not new_data.empty:
            ...
            buffer_positions = calculate_positions_and_velocity(buffer, reset_interval=reset_interval)
            ...
            give_drumtype_and_level(buffer_positions, peaks, drum_boundaries)
...
# Read and process both drum files with different drum boundaries
thread1 = threading.Thread(target=read_and_process_csv, args=(csv_file1, drum_boundaries_1))
thread2 = threading.Thread(target=read_and_process_csv, args=(csv_file2, drum_boundaries_2))
